---
name: houses-and-genealogy
description: "All 8 great houses — family members, Nyxen assignments, full genealogy trees (pre-crown + royal stem + all houses), queens register, blood index"
metadata: 
  node_type: memory
  type: project
  originSessionId: fece87fa-d74e-449c-bb83-eed0bc8e155a
  modified: 2026-09-01T17:42:33.498Z
---

**Slim-tree rule (locked):** Keep the royal line lean (Aegon→Rhaenyra density). Do not phone-book every cousin. Name riders only when story needs them.

**Call-pipe:** Numbered pipes begin under Aldren I. Ordinal map TBD.

**Spoiler rule:** Births and marriages only. No who wins which war.

## HOUSE BRENNAN OF FOLDHALL

### Who Lives Where (ordinary years)

| Person | Ordinary home |
|--------|---------------|
| King Aldren IX (Rodrik) & Queen Elyn | Foldhall |
| Rowan, Edwyn, Elyra | Rooms at Foldhall |
| Gareth | Foldhall and the road |
| Rhaenaya | Frostholt (wed to Osric Norvalen) |

### Family Tree: When the Telling Begins

```
                [earlier Aldrens...]
                       |
                ALDREN IX (Rodrik)  =  ELYN (Rathen)
                       |
          (no children yet at the opening)

    Rodrik's siblings (same generation):
    ROWAN (widower)     EDWYN (unmarried)   ELYRA
    GARETH (unmarried)  RHAENAYA = OSRIC NORVALEN
```

Nyxens of note: Hart (king), Briar (Edwyn), Lark (Elyra), Thorn (Gareth).
**Rowan has NO Nyxen** — he is the steward/ledger sibling.

### Family Tree: Later (as years turn, spoiler/future context only)

```
    ALDREN IX (Rodrik)  =  ELYN
              |
        +-----+-----+
        |           |
      EDRIC       HARWIN

    ROWAN ---- CAELEN, LERAEYNA
    EDWYN ---- TRISTAN, CASSIA
    ELYRA = ALTON RATHEN ---- ADERYN, ISABELLE
    RHAENAYA = OSRIC ---- ALARIC, BEATRICE
    GARETH (unmarried)
```

Nyxens of that generation: Ash (Edric), Storm (Harwin), Rook (Caelen), Frost (Alaric).

### Pre-Crown Line (Y0 → ~Y200)

Most spouses are Fold neighbors. Across two centuries only three noble-blood matches stick (one in, two out). Cold great-house offers refused.

```
Brennan the Farmer = Mera of the Fold                    [Y0 / Book 4]
 |
 └─ Robard = Elaine of Callenby
     |
     ├─ Godric (first) = Mara of Elmrick                 [HAULIER]
     │   |
     │   ├─ Steffon (first) = Maera Kelward     [FARMER / IN]
     │   │   |
     │   │   ├─ Ormund (first) = Bess of Quenby          [MILLER]
     │   │   │   |
     │   │   │   ├─ Harlan (first) = Ruth of Yarben      [FARMER]
     │   │   │   │   |
     │   │   │   │   ├─ Torrhen (first) → ALDREN I       [~Y200 / Book 3]
     │   │   │   │   ├─ Edwyn (second)                    [COURIER]
     │   │   │   │   └─ Rolf (third)                      [HUNTER]
     │   │   │   |
     │   │   │   ├─ Meggan (daughter) = Tommen Dorsen  [OUT]
     │   │   │   └─ Pate (third)                          [DELIVERY]
     │   │   |
     │   │   ├─ Jon (second)                              [COURIER]
     │   │   └─ Willem (third)                            [HUNTER]
     │   |
     │   └─ Willow (daughter) = Ser Renly Varen        [OUT]
     |
     └─ Hobb (second)                                     [COURIER]
```

### Royal Stem: Aldren I → V

```
ALDREN I (Torrhen) = Lysa Kelward
 |
 ├─ ALDREN II = Maris Rathen
 │   |
 │   ├─ ALDREN III (Leofric "the Adventurous") = (1st) Elen Varen; (2nd) Seraphine of Aurouvah
 │   │   |
 │   │   ├─ Aldric Brennan (by Elen; born Foldhall; d. "fever" — killed by blood ritual)
 │   │   ├─ ALDREN IV (by Seraphine) = Hilda Dorsen
 │   │   │   |
 │   │   │   ├─ ALDREN V = Catelyn Rathen        [Book 2 father]
 │   │   │   ├─ Elaena = Damon Varen
 │   │   │   ├─ Luthor = Alysanne Dorsen
 │   │   │   └─ Margaery = Brandon Norvalen
 │   │   |
 │   │   ├─ Jocelyn = Symond Kelward
 │   │   ├─ Garlan = Mina Selborn
 │   │   └─ Rhaena = Tristifer Rathen
 │   |
 │   ├─ Lyra = Edwyn Tarnell
 │   ├─ Humfrey = Pia Dorsen
 │   └─ Bethany = Rickard Norvalen
 |
 ├─ Osha = Horas Varen
 ├─ Donnel = Rhea Kelward                              [Eastverge appanage]
 └─ Tansy = Quentyn Tarnell
```

### Book 2 Generation (~Y400-450) → Aldren VI-VIII

```
ALDREN V = Catelyn Rathen
 |
 ├─ SEREN = Corwyn Yarben                              [firstborn / Book 2]
 │   |
 │   └─ ALDREN VI = Nysa Tarnell
 │       |
 │       ├─ ALDREN VII = Freya Norvalen
 │       │   |
 │       │   ├─ ALDREN VIII = Gwyn Selborn
 │       │   ├─ Alys = Jonos Kelward
 │       │   └─ Robert = Lysa Dorsen
 │       |
 │       ├─ Jeyne = Ronnel Rathen
 │       ├─ Willam = Olenna Selborn
 │       └─ Cersei = Paxter Varen
 |
 ├─ OSBERT                                                [first son / Book 2]
 ├─ Randa = Cregan Norvalen
 └─ Martyn = Helicent Tarnell
```

### Book 1 Generation (~Y500)

```
ALDREN VIII = Gwyn Selborn
 |
 ├─ ALDREN IX (Rodrik) = ELYN Rathen
 │   |
 │   ├─ EDRIC = (betrothed) ROSELYN Varen
 │   ├─ HARWIN
 │   └─ (survivor) ALDREN X = (queen TBD)
 │       |
 │       └─ ELOWEN
 |
 ├─ ROWAN                    [widower / no Nyxen / steward]
 ├─ EDWYN                    [unmarried / rides Briar / performer]
 ├─ ELYRA = (later) ALTON Rathen
 ├─ RHAENAYA = OSRIC Norvalen
 │   |
 │   ├─ ALARIC
 │   └─ BEATRICE
 |
 └─ GARETH
```

## HOUSE RATHEN OF STILLFORD

Former kings of Ratherlon. Merelands paramounts. Queen Elyn is that blood.

```
Harwin Rathen = Lydia
 |
 ├─ Maris = ALDREN II                                    [queen]
 └─ Tristifer = Rhaena of Brennan
     |
     └─ Edmure = Sella Dorsen
         |
         └─ Hoster = Anya
             |
             ├─ Catelyn = ALDREN V                        [queen]
             └─ Ronnel = Jeyne of Brennan
                 |
                 └─ Edric Rathen = Mira
                     |
                     ├─ ELYN = ALDREN IX
                     └─ Living lord = Helen Pellridge
                         |
                         ├─ ALTON = (later) ELYRA
                         └─ EDMUND
```

## HOUSE VAREN OF ROSEGATE

```
Horas Varen = Osha of Brennan
 └─ Garth = Rosa
     ├─ Elen = ALDREN III (consummated; bore Aldric Brennan at Foldhall; Aldric killed by blood ritual; Elen wed within Vine Shore after Leofric fled)
     └─ Damon = Elaena of Brennan
         └─ Paxter = Cersei of Brennan
             └─ Tytos = Lanna
                 └─ ROLAND = Sarra
                     └─ ROSELYN = (betrothed) EDRIC
```

## HOUSE NORVALEN OF FROSTHOLT

```
Rickard = Bethany of Brennan
 └─ Brandon = Margaery of Brennan
     └─ Cregan = Randa of Brennan
         └─ Torrhen Norvalen = Skadi
             ├─ Freya = ALDREN VII                        [queen]
             └─ Osric's father = Ylva
                 └─ OSRIC = RHAENAYA of Brennan
                     ├─ ALARIC (rider, Nyxen Frost)
                     └─ BEATRICE
```

## HOUSE KELWARD OF EASTGIRT

```
Lord Kelward = Maera
 ├─ Lysa = ALDREN I                                       [queen]
 └─ Rhea's brother (lord)
     └─ Rhea = Donnel of Brennan                          [Eastverge appanage]
         └─ Symond = Jocelyn of Brennan
             └─ Jonos = Alys of Brennan
                 └─ SIMON = Cora   [Book 1 lord of Eastgirt]
```

## HOUSE TARNELL OF SALTCROWN

```
Quentyn = Tansy of Brennan
 └─ Edwyn = Lyra of Brennan
     └─ Alyn = Tess
         ├─ Nysa = ALDREN VI                              [queen]
         ├─ Helicent = Martyn of Brennan
         └─ Harbor line
             └─ Living lord
                 └─ HELENA  [at Foldhall, settler of quarrels]
```

## HOUSE DORSEN OF RIVERMARK

```
Lord Dorsen = Willa
 ├─ Hilda = ALDREN IV                                     [queen]
 └─ Pia's brother (lord)
     ├─ Pia = Humfrey of Brennan                          [married out; children are Brennans]
     └─ (lord)
         └─ (lord)
             └─ Alden = Sella
                 ├─ Lysa = Robert of Brennan               [married out; children are Brennans]
                 └─ Living lord = Maren
```

## HOUSE SELBORN OF PINEFALL

```
Lord Selborn = Fern
 └─ Mina = Garlan of Brennan
     └─ Olenna's father
         └─ Olenna = Willam of Brennan
             └─ Gwyn's father = Ash of the pines
                 ├─ Gwyn = ALDREN VIII                    [queen]
                 └─ Living lord
```

## WHO HAS BRENNAN BLOOD (send-to index)

| Hall | How |
|------|-----|
| Stillford | Queens Maris, Catelyn, Elyn; Rhaena, Jeyne; later Elyra |
| Rosegate | Osha, Elen (+ Aldric Brennan, her son by Leofric — d. blood ritual), Elaena, Cersei; Roselyn betrothal |
| Frostholt | Bethany, Margaery, Randa, Freya; Rhaenaya |
| Eastgirt | Lysa, Donnel appanage, Jocelyn, Alys |
| Saltcrown | Tansy, Lyra, Nysa, Martyn; Helena |
| Rivermark | Hilda, Humfrey, Luthor, Robert |
| Pinefall | Garlan, Willam, Gwyn |

## PRINCIPAL CAST & RIDERS

**CORRECTED per birth-order-locked.md:** Rowan = non-rider/steward/ambitious/father of Caelen. Edwyn = rider (Briar)/performer.

| Person | Role | Rider | Nyxen | Notes |
|--------|------|-------|-------|-------|
| King Aldren IX (Rodrik) | King | ✓ | Hart | ~2-4y crowned at story open |
| Queen Elyn Rathen | Queen | ✗ | | Suicide when everything spirals |
| Prince Edric | Diplomatic twin | ✓ | Ash | Betrothed to Roselyn |
| Prince Harwin | Warrior twin | ✓ | Storm | Fetches Oak for Twin War |
| Prince Rowan | King's brother | ✗ | | Ambitious; orders Caelen out; steward/ledger |
| Prince Edwyn | King's brother | ✓ | Briar | Performer; Littlefinger parallel (fun, no evil motive) |
| Princess Elyra | King's sister | ✓ | Lark | → Edric's camp |


| Princess Rhaenaya | King's sister | ✗ | | Wed Osric Norvalen |
| Prince Gareth | King's brother | ✓ | Thorn | → Harwin's camp; cold by nature |

### Next Generation / Cousins

| Person | Rider | Nyxen | Notes |
|--------|-------|-------|-------|
| Caelen Brennan (Rowan's son) | ✓ | Rook | Disobeys Rowan → Edric |
| Leraeyna Brennan (Rowan's) | ✗ | | |
| Tristan Brennan (Edwyn's) | ✓ | TBD | → Edric |
| Cassia Brennan (Edwyn's) | ✓ | TBD (Wild) | → Edric |
| Aderyn Rathen (Elyra's) | ✓ | TBD | → Edric |
| Isabelle Rathen (Elyra's) | ✗ | | |
| Alaric Norvalen (Rhaenaya's) | ✓ | Frost | → Harwin |
| Beatrice Norvalen (Rhaenaya's) | ✗ | | |




### End of Book Only (spoilers)

| Person | Notes |
|--------|-------|
| King Aldren X | Survivor; identity unresolved |
| Princess Elowen | His daughter; fire scene final pages |
| Oak (Nyxen) | VII's mount → second rider VIII; grief-survivor; Harwin fetches for Twin War |

### Active Riders at War's Start (11)

Aldren IX (Hart), Edric (Ash), Harwin (Storm), Edwyn (Briar), Elyra (Lark), Gareth (Thorn), Caelen (Rook), Aderyn (TBD), Alaric (Frost), Tristan (TBD), Cassia (TBD).

**Rowan is NOT a rider.** He has no Nyxen.

**Alys (Kelward) and Robert (Dorsen) are NOT riders.** As the younger siblings of King Aldren VIII, their Nyxen scent was too weak to bond with a beast. They provide major political and geographical clout for their respective regions (Eastgirt and Rivermark), but they do not field Nyxens.

